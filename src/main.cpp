...
void initDSP()
{
  ...
  ledcWrite(0, 50); /* Screen brightness can be modified by adjusting this parameter. (0-255) */
...
void loadGIFT()
{
  ...
  File gifFile1 = SD.open(GIF_FILENAME1, "r");
  gd_GIF *gif1 = gifClass1.gd_open_gif(&gifFile1);
  uint8_t *buf1 = (uint8_t *)malloc(gif1->width * gif1->height);
  int16_t x1 = (gfx->width() - gif1->width) / 2;
  int16_t y1 = (gfx->height() - gif1->height) / 2;
  int32_t res1;
  ...
  while (1)
  {
    ...
    {
      gfx->drawIndexedBitmap(x1, y1, buf1, gif1->palette->colors, gif1->width, gif1->height);
    }
    yield();
  }
}
